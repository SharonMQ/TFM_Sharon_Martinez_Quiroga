...
```{r}
datos_Met_def<-datos_Met_def_se[, 1:274]
...
Finalmente se va a ordenar las bases de datos para que las columnas estén en el mismo orden, como CNA y Met ya tienen el mismo orden, se ordenará mRNAseq para que coincida con las dos anteriores.
...
Si se visualiza el data frame "datos_mRnaseq_def" se ve que las 4 primeras muestras que aparecen en "datos_CNA_def" aparecen a partir de la columna 9 de "datos_mRnaseq_def", por lo que vamos a comprobar si se han guardado las muestras en el orden correcto.
