...
El filtrado biológico consistirá en seleccionar aquellos genes que tienen una expresión diferencial entre las muestras normales y las patológicas. Para no perder demasiados genes y que no se obtengan buenos resultados en la integración, se guardará aquellos genes con una significación menor que 0.10. No se hará con 0.25 de nivel de significación como indica Mo Q et al[] por falta de tiempo para realizar la integración con todos esos datos.
...
Finalmente con la función `decideTest`, indicandole el nivel de significación que queremos, que en este caso es 0.10 ya que solo deseamos filtrar los genes. No se ha usado 0.25 como en el papeline por falta de tiempo. Igualmente se obtendrán los significativos al 0.05 para comparar los resultados con los de iClusterBayes.

* _Filtrado_
```{r}
genes_filtrado<-decideTests(efit, p.value = 0.10)
...
Se guardará los genes con sus log(FPKM) con expresión diferencial.
...
Finalmente se guardan los genes a usar con sus datos, pero solo las muestras patológicas:

```{r}
resultdir<- file.path("~/Documentos/6to cuatrimestre/TFM/Workflows/Workflow/Datos", "GE_filt.csv")
write.csv(GE_filt[,42:498], resultdir, row.names = TRUE)
resultdir<- file.path("~/Documentos/6to cuatrimestre/TFM/Workflows/Workflow/Datos", "GE_trans.csv")
write.csv(GE_trans[,42:498], resultdir, row.names = TRUE)
```
