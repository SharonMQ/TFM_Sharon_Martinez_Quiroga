...
5. **Filtrado biológico para integración**
  * 5.1 _Análisis de expresión diferencial significación 0.25_
  * 5.2 _Seleción del 30% de Top genes_
6. **Análisis de expresión diferencial significación 0.05**
...
### 5.1.Análisis de expresión diferencial significación 0.25:

El filtrado biológico consistirá en seleccionar aquellos genes que tienen una expresión diferencial entre las muestras normales y las patológicas. Al ser solo un filtrado se obtendrán primero los genes con una significación menor que 0.25, y posteriormente se seleccionarán el 30% de los genes con menor p-valor ajustado. No se usará el 25% de top genes porque da un número de genes para integrar mucho menor que el usado en Mo et al, y los genes seleccionados tienen un p-valor<$10^{-11}$
...
Finalmente con la función `decideTest`, indicandole el nivel de significación que queremos, que en este caso es 0.25, ya que solo deseamos filtrar los genes y necesitamos un número elevado de genes para la integración.  Esta función lo que hace es crear una tabla con posibles valores, para cada gen, de -1(genes con expresión reprimida en células tumorales), +1(genes con expresión elevada  en células tumorales) y 0(genes con misma expresión en ambas condiciones). Se otorgará cada valor comparando el p-valor obtenido con Bayes y el nivel de significación.
...
Ahora se guardan los resultados completos con sus p-valores:

```{r}
write.fit(vfit, genes_filtrado, file="genes_filtrado.txt")
```

También se guardan los genes diferencialmente expresados con sus log(FPKM) para luego elegir el 30% más significativos. 
...
### 5.2 Seleción del 30% de Top genes

Ahora se guardará, como datos filtrados para la integración, el 30% de los genes expresados diferencialemte más significativos(menor p-valor ajustado), con sus log(FPKM), para ello se usará la función `topTable` de `limma`. 
...
```{r}
selec_filt<-rownames(top_genes_filt)
filt<-rownames(GE_filt)
selec_pos<-vector()
for (i in 1:length(selec_filt)){
  for (j in 1:length(filt)){
    if (selec_filt[i]==filt[j]){
      selec_pos<-c(selec_pos, j)
    }
  }
}
selec_top_genes<-GE_filt[selec_pos,]
```
...
Por tanto, se han seleccionado correctamente.
...
```{r}
resultdir<- file.path("~/Documentos/6to cuatrimestre/TFM/Workflows/Workflow/Datos", "Top_genes.csv")
...
Expresión diferencial para la ómica de forma independiente a un nivel de significación de 0.05. 

```{r}
genes_trans<-decideTests(efit, p.value = 0.05)
(res<-as.data.frame(summary(genes_trans)))
```
...
GE_trans<-datos_mRnaseq[dif_GE_trans,]
```

Se aelecciona los Top genes al 30%:

```{r}
dif_exp<-res[1,3]+res[3,3]
dif_exp
```
...
```{r}
top_genes_trans<-topTable(efit, coef="NormVSTum", sort.by = "p", number = topgenes)
head(top_genes_trans)
```

A continuación, se guarda en un dataframe los Top Genes y sus correspondientes $log_2(FPKM)$:

```{r}
selec_trans<-rownames(top_genes_trans)
trans<-rownames(GE_trans)
selec_pos<-vector()
for (i in 1:length(selec_trans)){
  for (j in 1:length(trans)){
    if (selec_trans[i]==trans[j]){
      selec_pos<-c(selec_pos, j)
    }
  }
}
selec_top_genes<-as.data.frame(GE_trans[selec_pos,])
```
...
Veremos primero un heatmap con 500 genes diferencialmente expresados, estos genes estarán tanto en los genes filtrados para usarlos en la integración como para los que se han obtenido con un nivel de significación menor a 0.05. Solo se observarán 100 muestras porque en realidad el observar estos resultados de diferencia de expresión entre individuos normales y con tumor no es nuestro mayor interes en el estudio.
```{r}
...
heatmap.2(selec_top_genes[1:500,],labCol=grupo, col=morecols,trace="none", main="Top 500 most variable genes across samples",ColSideColors=col_muestra,scale="row")
```

A simple vista con el heatmap es difícil diferenciar los subtipos dentro del grupo de muestras patológicas, aún así parecen haber por lo menos tres subtipos que pueden afectar a la prognosis. 
